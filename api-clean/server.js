...
console.log('🚀 CleanPay API System Starting - Secure & Functional');
...
// In-memory API key store (secure for staging)
const apiKeys = new Map();

// Initialize with working test keys
function initializeApiKeys() {
    ...
    console.log(`✅ Initialized ${keys.length} API keys`);
}
...
app.get('/api/status', (req, res) => {
    res.json({
        ...
        timestamp: new Date().toISOString(),
        features: {
            authentication: 'secure-keys',
            purchase: 'direct-processing',
            demo: 'spending-controls'
        }
    });
});

// List API keys (no auth required for staging convenience)
...
app.post('/v1/purchase-direct', (req, res) => {
    try {
        ...
        const processingDelay = 50 + Math.random() * 100; // 50-150ms
...
// ====================================
// DEMO SPENDING CONTROLS (COPY WORKING LOGIC)
// ====================================
...
// Spending validation with anti-spam
...
const routes = [
    '/pricing', '/comparison', '/docs', '/demo', '/security', 
    '/status', '/dashboard', '/auth'
];
...
app.listen(port, () => {
    ...
    console.log(`🔑 API Keys: ${apiKeys.size} initialized`);
    console.log('✅ Secure, functional, and ready for AI agents');
});
